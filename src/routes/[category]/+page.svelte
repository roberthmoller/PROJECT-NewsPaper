<script lang="ts">
	import { StoryListItem } from '$lib/components';
	...
	// derived(page, (value) => )
	const pageNumber = parseInt($page.url.searchParams.get('page') ?? '1');
	...
	<nav class="grid-cols-4 mx-auto my-2">
		<form method="POST">
			<sup>
				...
				<button class="hover:underline uppercase" formaction="?/previous&page={pageNumber}"
					><b>previous</b></button
				>
				| Page Nº{pageNumber}
				|
				<button class="hover:underline uppercase" formaction="?/next&page={pageNumber}"
					><b>next</b></button
				>
