...
import type { Comment, CommentTree, Fetch } from '$lib/types';
...
export class HackerNewsApi {
	...
	/**
	 * Get one level of comments for given item item
	 * @param item The item to get the comments for
	 * @returns The comments for the item
	 */
	async comments(item: Item): Promise<Comment[]> {
		if (!item.kids || item.kids.length == 0) return [];
		const children = await Promise.all(item.kids.map(kid => this.item(kid)));
		return await children.map(comment => comment as unknown as Comment);
	}
}
